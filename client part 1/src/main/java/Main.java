...
public class Main {
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        for (int i = 0; i < FIRST_PHASE_NUM_THREADS; i++) {
            SkiersApiConsumer firstPhaseConsumer = new SkiersApiConsumer(queue, NUM_REQUESTS_PER_THREAD, resultData, firstPhaseCompleted, allCompleted);
        ...
        for (int j = 0; j < SECOND_PHASE_NUM_THREADS; j++) {
            SkiersApiConsumer secondPhaseConsumer =
                    new SkiersApiConsumer(queue, NUM_REQUESTS_PER_THREAD, resultData, secondPhaseCompleted, allCompleted);
