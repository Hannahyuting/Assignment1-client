...
public class SkiersApiConsumer implements Runnable {
    ...
    public SkiersApiConsumer(BlockingQueue<SkierLiftRide> queue, int numOfRequests, ResultData resultData, CountDownLatch currentPhaseCompleted, CountDownLatch allCompleted) {
        this.queue = queue;
        ...
        for (int i = 0; i < numOfRequests; i++) {
            ...
            try {
                ...
                if (apiResponse.getStatusCode() == 201 || apiResponse.getStatusCode() == 200) {
                    ...
                    while (apiResponse.getStatusCode() != 201 || apiResponse.getStatusCode() != 200) {
